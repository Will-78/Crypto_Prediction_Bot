...
async def insert_post(post_id, title, sentiment):
    ...
    c = conn.cursor()
    c.execute(
        "INSERT INTO posts (id, title, sentiment) VALUES (?, ?, ?)",
...
async def cache_prediction(crypto_id, response):
    ...
    c.execute(
        "INSERT INTO predictions(crypto_id, response) VALUES(?, ?)",
        (crypto_id, response),
    )
    conn.commit()
    conn.close()
...
async def insert_currency(symbol):
    conn = sqlite3.connect("crypto_trading.db")
    c = conn.cursor()
    c.execute("INSERT INTO currencies(symbol) VALUES(?)", (symbol,))
    conn.commit()
    conn.close()
